...
def multiple_plot(average_vals_list, std_dev_list, traj_list, other_labels, env_name, smoothing_window=5, no_show=False, ignore_std=False, limit=None):
    ...
    for average_vals, std_dev, label, trajs in zip(average_vals_list, std_dev_list, other_labels, traj_list):
        ...
        color_index += 1
        cum_rwd_1, = plt.plot(trajs, rewards_smoothed_1, label=label, color=fill_color[0])
        if not ignore_std:
...
multiple_plot(avg_rets, std_dev_rets, trajs, args.labels, args.env_name, smoothing_window=args.smoothing_window, no_show=args.save, ignore_std=args.ignore_std, limit=args.limit)
